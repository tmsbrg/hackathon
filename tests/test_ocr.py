...
class OcrTests(unittest.TestCase):
    ...
    def test_collect_ocr_findings_uses_tesseract_for_all_documented_image_extensions(self, run_command: mock.Mock) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            run_command.side_effect = [cli.CommandResult(0, "", "", False) for _ in range(len(images) * 2)]
    ...
    @mock.patch("doc_triage.cli.run_command")
    def test_collect_ocr_findings_uses_pdf_ocr_for_all_documented_pdf_extensions(self, run_command: mock.Mock) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            target = Path(tmpdir)
            work_dir = target / "ocr"
            work_dir.mkdir()
